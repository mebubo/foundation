...
object FunctionExercises extends FunctionToImpl {
  ...
  def tripleList(xs: List[Int]): List[Int] = xs.map(triple)
  ...
  val move2: (Boolean, Int) => Int = (b, x) => move(b)(x)

  val move3: Boolean => Int => Int = move _

  // 1f. Implement applyMany
  // such as applyMany(List(_ + 1, _ - 1, _ * 2))(10) == List(11, 9, 20)
  def applyMany(xs: List[Int => Int]): Int => List[Int] = y => xs.map(_(y))
  ...
  // hint: use updateAge with one of the polymorphic function we just saw
  def getUsers: List[User] = updateAge(identity)
  ...
  val doubleInc: Int => Int = double andThen inc

  // 2i. Same for f(x) = 2 * (x + 1)
  val incDouble: Int => Int = double compose inc
  ...
  def forAll(xs: List[Boolean]): Boolean = xs match {
    case Nil => true
    case head :: next => head && forAll(next)
  }
  ...
  def forAll2(xs: List[Boolean]): Boolean = foldRight(xs, true)((h, t) => if (!h) false else t)
